...
class	App:
    def	__init__(self,	window,	window_title,	video_source=0):
        ...
        self.btn_snapshot.grid(row=12, column=3, rowspan=7)#anchor=tkinter.CENTER, expand=True)
        # Button for applying a gaussian filter
        ...
        #	After	it	is	called	once,	the	update	method	will	be	automatically	called	every
        self.delay = 15
        self.update()
    ...
    def	get_frame(self):
        if	self.vid.isOpened():
            ...
            if ret:
                ...
                return (ret, frame,  self.frame1) #cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            else:
                return (ret, None)
        else:
            return	(ret, None)
